...
\section{Statistical model and algorithm} \label{sec:models}
...
The five models can be divided in two categories.
...
\begin{table*}
  ...
  \caption{\label{tab:losses} Different change-point models provided
    ...
    Complexity is for computing best split on a segment with $N$ data.
  }
\end{table*}
...
\paragraph{Square loss, normal change in mean,} \verb|distribution.str="mean_norm"|.
The segment-specific parameter $\theta\in\mathbb R$ is the mean.

\paragraph{Normal change in mean and variance,} \verb|distribution.str="meanvar_norm"|.
The total Gaussian negative log likelihood is used as the loss function for \verb
The segment-specific parameter $\theta\in\mathbb R^2$ is the mean and variance.
...
\paragraph{Poisson change in rate parameter (mean and variance),} \verb|distribution.str="poisson"|.
The total Poisson loss is used for 
...
The binary segmentation algorithm recursively computes the following quantities for each $k\in\{2,\dots,S\}$.
