...
\Abstract{ Binary segmentation is a classic algorithm for detecting
  ...
  \pkg{changepoint} (\proglang{R} package) are substantially slower.
  ...
  We discuss some C++ coding techniques
  ...
  segmentation in other packages: \pkg{ruptures}, \pkg{fpop}, \pkg{changepoint} and
  \pkg{wbs}. }
...
\begin{table*}
  ...
  \begin{tabular}{ccccccc}
    ...
    \texttt{laplace} & Absolute & Laplace & yes & yes & 2 & $O(S\log S)$ \\
  \end{tabular}
...
% https://github.com/HaotianXu/changepoints does not have binary segmentation.
  version & 2022.3.29 & 2.2.3 & 1.4 & 2019.8.26 & 1.1.9\\
\hline
...
ex6_binsegRcpp <- binsegRcpp::binseg("mean_norm", ex6_df$value, max.segments = 4)
