...
\section{Distributions and loss functions}

We assume there is a sequence of $N$ data $x_1,\dots,x_N\in\mathbb R$,
in which we want to find abrupt changes.  We
further assume that on each segment there is a constant model
parameter $\theta$, typically the mean (and perhaps
variance/scale). We would like to find a sequence of change-points,
such that each segment has a parameter $\theta$ which minimizes a loss
function $\ell(\theta,x)$. The loss functions that we consider to
minimize can be interpreted as maximizing the log likelihood of
several common distributions (Normal, Poisson, Laplace).
...
location parameter $\mu\in\mathbb R$, and scale parameter $b\in\mathbb
R$, is
\begin{equation}
  \frac{1}{2b} \exp\left(
  \frac{|\mu-x|}{-b}
...
A \code{class absDistribution} implements the common operations for the two loss functions, including parameter estimation via a cumulative median algorithm, which is required to efficiently compute the optimal split point on a segment.
To implement the cumulative median for $N$ data in $O(N\log N)$ time, we used a modified version of the L1 loss minimization code proposed by \citet{Drouin2017mmit}.
For a segment from $j$ to $e$ ($1\leq j\leq e\leq N$), the estimated parameters are
\begin{eqnarray}
  \mu^* &=& \argmin_\mu \sum_{i=j}^e w_i |\mu-x_i| \\
  b^* &=& \frac{\sum_{i=j}^e w_i |\mu^*-x_i|}{\sum_{i=j}^e w_i}.
\end{eqnarray}
Note that for an even number of data, there is a range of $\mu^*$ values which minimize the absolute error. In that case, we define $\mu^*$ as the value in the middle of the range of values which minimize the absolute error (this is the traditional definition of the sample median).

\subsection{Loss functions based on cumulative sums}

There are three models which use the cumulative sum trick to find the
best split for $N$ data in $O(N)$ time.
In particular, we define the vectors $W,X,Y\in\mathbb R^{N+1}$ as follows:
\begin{eqnarray}
  \text{Cumulative weights: } W_0=0,\,&& \forall t\in\{1,\dots,N\},\, W_t=\sum_{i=1}^t w_i,\\
  \text{Cumulative sum: }X_0=0,\,&& \forall t\in\{1,\dots,N\},\, X_t=\sum_{i=1}^t w_i x_i,\\
  \text{Cumulative squares: }Y_0=0,\,&& \forall t\in\{1,\dots,N\},\, Y_t=\sum_{i=1}^t w_i x^2_i.
\end{eqnarray}
...
\end{equation}
For a segment from $j$ to $e$ ($1\leq j\leq e\leq N$), we use the notation $W_{j,e}=W_j-W_{e-1}$ for the sum of the weights over the segment, and similarly for $X_{j,e}$ and $Y_{j,e}$. Then the mean on the segment from $j$ to $e$ can be computed via
\begin{equation}
  \mu^* = X_{j,e}/W_{j,e}.
\end{equation}
...
\end{equation}
For a segment from $j$ to $e$ ($1\leq j\leq e\leq N$), the variance is estimated via
\begin{equation}
...
segment-specific parameter $\theta\geq 0$ is the rate.  The Poisson
...
\begin{equation}
  \ell_{\texttt{poisson}}(\theta,x)=\theta - x\log \theta.
\end{equation}
...
This section gives the equations for the binary segmentation algorithm.

\subsection{Min loss and difference}
...
\end{equation}
The minimization problem is over the model parameter $\theta$ (which affect the center and possibly scale/variance).
...
\end{equation}
Ideally, the minimization above takes $O(1)$ constant time, if the set
...
remove/insert operations should be sub-linear. The list container
...
In general, after running binary segmentation up to $S$ segments, we
can compute a table of estimated means (one row for each segment), for
any number of segments $k\in\{1,\dots,S\}$, using the following algorithm.
...
to missing (\code{NA}), based on values in \code{invalidates.index}
and \code{invalidates.after} columns.  Finally, sort the \code{end}
