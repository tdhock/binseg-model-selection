...
We compared our proposed implementation to these baselines in terms of functionality, correctness, and efficiency.

The \pkg{fpop} package in R contains the \code{fpop::multiBinSeg}
function, which implements the square loss for multi-dimensional data
\citep{Maidstone2017}.

The \pkg{wbs} package in R contains the \code{wbs::sbs} function,
which implements the square loss for univariate data
\citep{Baranowski2019}.  Since it uses recursive function calls, it
does not support the max segments parameter $K$, and instead always
computes the full path of models (from 1 to $N-1$ change-points).

The \code{changepoint::cpt.mean(method="BinSeg")} \citep{Killick2014,Killick2022}.
