...
\subsection{Distributions and loss functions}
...
\paragraph{Square loss, normal change in mean.}
The total square loss is used for \verb|distribution.str="mean_norm"|.
The segment-specific parameter $\theta\in\mathbb R$ is the mean.

\paragraph{Normal change in mean and variance.}
The total Gaussian negative log likelihood is used as the loss function for \verb|distribution.str="meanvar_norm"|.
...
\paragraph{Poisson change in rate parameter (mean and variance).}
The total Poisson loss is used for \verb|distribution.str="poisson"|

\paragraph{L1 loss, Laplace change in median.}
The total absolute error is used for \verb|distribution.str="l1"|

\paragraph{Laplace change in median and scale.}
The total Laplace negative log likelihood is used for
\verb|distribution.str="laplace"|
