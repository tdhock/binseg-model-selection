...
CUM_DIST(meanvar_norm,
 ...
 , true)
\end{verbatim}
The code below declares three loss functions based on the cumulative sum: \code{mean_norm} is the residual sum of squares (RSS), \code{poisson} loss is for count data, and \code{meanvar_norm} is the normal model with change in mean and variance. The \code{COMPUTE} parmater specifies the loss, in terms of variables such as \code{mean} and \code{sum} which are computed in a \code{class CumDistribution} which contains the logic common to these loss functions.
