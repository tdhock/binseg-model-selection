...
\section{Introduction: change-point algorithms and software} \label{sec:intro}
...
\citet{segment-neighborhood}, we can compute optimal change-points for a given cost
function, and a sequence of model sizes $k\in\{1,\dots,S\}$, in $O(N^2 S)$ time.
%\citep{Jackson2005}
...
Because binary segmentation is a heuristic, it is not guaranteed to compute the globally optimal model that is computed by dynamic programming.
There are several well-known types of data where binary segmentation fails, such as abrupt changes up and then back down, which can be difficult because of the greedy iterative nature of the algorithm.
Nevertheless, binary segmentation is widely used in many practical applications, because its best-case run-time is extremely fast.
...
$O(N^{1.4})$ time complexity (see Section~\ref{sec:not-best-time}), which is slower
...
corresponding weight $w_i>0$, which is multiplied by the loss function,
...
To compute the sequence of loss differences $D_{j,e}(j),\dots,D_{j,e}(e-1)$, we need to compute the sequence of values $L_{j,j}, \dots, L_{j,e-1}$ (loss values for possible new segments before candidate split points), and the sequence of values $L_{j+1,e}, \dots, L_{e,e}$ (loss values for possible new segments after candidate split points).
...
\begin{table}
  ...
  \begin{tabular}{c|cc|cc|cc}
    Splits: & \multicolumn{2}{c|}{one split, iteration $k$} & \multicolumn{2}{c|}{$S$ equal splits (best)} & \multicolumn{2}{c}{$S$ unequal splits (worst)} \\
    operation: & insert &
    %remove &
...
Next, the total loss is computed by adding the best loss decrease $d^*_k$ to the best loss $L_{k-1}$ at the previous iteration, 
\begin{equation}
  \mathcal L_k=\mathcal L_{k-1} +d^*_k.
\end{equation}

Next, we construct a set of two new segments to consider:
\begin{equation}
  \label{eq:Nk}
  \mathcal N_k=\{
  (j^*_k, t^*_k),
  ...
  \}.
\end{equation}
New segments in $\mathcal N_k$ could be too small to split, if there is only one data point on that segment.
...
We aim to emphasize the unique features of \pkg{binsegRcpp}, relative to previous software packages.

\subsection{Retreival of segment-specific model parameters}
One unique feature of \pkg{binsegRcpp} is easy retreival of segment-specific model parameters.
...
\section{Empirical time complexity analysis} \label{sec:time-complexity}
This section presents results from empirical timings of the proposed
\pkg{binsegRcpp} package, along with baselines. Unless otherwise
stated, each empirical measurement was conducted 10 times on a 2.40GHz Intel(R)
Core(TM)2 Duo CPU P8600.
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:heap-refs-binsegRcpp}
    ...
    Black curves are empirical measurements for \pkg{binsegRcpp} with two containers (median line and min/max band), and violet curves are references.
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:heap-refs-other}
    Estimating best-case asymptotic complexity for time (seconds) and memory (kilobytes).
    Black curves are empirical measurements for \pkg{changepoint} and \pkg{ruptures} (median line and min/max band), and violet curves are references.
...
\subsection{Timings and throughput using the square loss}
...
Whereas there are not very large timing differences for small data sizes (all work in less than one second for up to 1000 data), there are large differences for data sizes of several thousand data or larger.
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:heap-pred} Comparing best-case computation time of \pkg{binsegRcpp} with three containers, and two baselines: \pkg{changepoint} and \pkg{ruptures} (median line and min/max band).
    Square loss and best case data were used (integers from 1 to $N$, result in equal splits).
    N shown for each method is the estimated throughput in a time limit of 5 seconds.
  }
\end{figure}

\subsection{Square loss comparisons for best case, worst case, flat data}
To demonstrate that \pkg{binsegRcpp} achieves state-of-the-art
computation time, we conducted empirical timings on other
implementations of the square loss (\code{fpop::multiBinSeg}, \code{wbs::sbs},
\code{blockcpd::heap}). This loss function is
based on the cumulative sum, so is expected to achieve $O(N\log S)$
time complexity in best case data.  To check this expectation,
Figure~\ref{fig:timings} shows timings for running up to $S=N/2$
segments, for the square loss in three different data sets (best case,
worst case, flat).
\begin{description}
\item[Best case:] we used the sequence $1,\dots,N$.
\item[Flat case:] we used $N$ random numbers drawn from the standard normal distribution.
\item[Worst case:] we used $N$ numbers alternating between 0 and 1.
\end{description}
It is clear that \code{fpop::multiBinSeg} and \code{wbs::sbs}
have very similar asymptotic timings as \pkg{binsegRcpp} in all three
cases. In particular, these observations confirm our expectation of
$O(N\log S)$ time complexity in best case data. In the worst case,
all packages show the same slope (indicating quadratic time), except
\pkg{changepoint} (which has a larger slope, indicating cubic time).
...
\subsection{Poisson and Normal change in mean and variance models}
We also investigated the the normal change in mean and
variance model (Figure~\ref{fig:timings-meanvar_norm}), and Poisson model
(Figure~\ref{fig:timings-poisson}), which are
implemented by \code{blockcpd::heap} and \pkg{changepoint}. Again, these are expected to
achieve $O(N\log S)$ time complexity in $N$ best case data up to $S$ segments.
We ran up to $S=N/2$ segments, so $O(N\log N)$ complexity was expected.
To check this expectation, Figures~\ref{fig:timings-poisson}--\ref{fig:timings-meanvar_norm} show timings for
these loss functions with different kinds of data (best case, worst case,
flat).
\begin{description}
\item[Best case:] we used the sequence $1,\dots,N$.
\item[Flat case:] we used $N$ random numbers drawn from the standard normal distribution (normal model) or the Poisson distribution with rate 100 (Poisson model).
\item[Worst case:] we used $N$ numbers which repeat the sequence $0,1$ (Poisson model) or $0,1,10,11$ (normal model).
\end{description}
In Figures~\ref{fig:timings-meanvar_norm}--\ref{fig:timings-poisson}, it is clear
that \code{blockcpd::heap} has the same asymptotic slope, but much
larger constant factors, when compared with \pkg{binsegRcpp}.
Similar to results for the square loss, \pkg{changepoint} has larger slopes than \pkg{binsegRcpp} on the log-log plot, which indicate a larger asymptotic time complexity class.
Also consistent with results for the square loss, Figure~\ref{fig:timings-meanvar_norm} shows that \pkg{ruptures} has a larger slope than \pkg{binsegRcpp} in best and flat cases, indicating a larger asymptotic time complexity class.
These data indicate that \pkg{binsegRcpp} provides a state-of-the-art implementation of the models for change in Poisson rate, and change in Normal mean and variance.
\begin{figure}[t!]
  \centering
...
\subsection{Comparison using L1 loss}
...
We expected that \pkg{binsegRcpp} should have similar computation times as \pkg{ruptures} for various kinds of data (best case, flat, worst case).
In particular, these are expected to
achieve $O(N\log N\log S)$ time complexity in best case data.
To check this expectation, Figure~\ref{fig:timings-laplace} shows timings for
the L1 loss in three different data sets.
\begin{description}
\item[Best case:] we used the sequence $1,\dots,N$.
\item[Flat case:] we used $N$ random numbers drawn from the standard normal distribution.
\item[Worst case:] we used the sequence $x_i=2^{-i}$ for all $i\in\{1,\dots,N\}$.
\end{description}
Figure~\ref{fig:timings-laplace} shows that in the best case, \pkg{ruptures} has slightly larger asymptotic slope, and much larger constant factors, than \pkg{binsegRcpp}.
It also shows that \pkg{ruptures} has larger constant factors than \pkg{binsegRcpp}, in the flat and worst cases.
These data refute our expectation of similar performance between \pkg{ruptures} and \pkg{binsegRcpp}.
Overall, these data indicate that \pkg{binsegRcpp} provides a state-of-the-art implementation of the L1 loss.

\subsection{Comparing time complexity of different loss functions}
We expected that the L1 loss (Laplace change in median model)
would be asymptotically slower than the Normal and Poisson models,
for best case data (due to the log-linear rather than linear time sub-routine for computing the best split per segment).
To check this expectation, Figure~\ref{fig:compare-distributions} shows combined results from the experiments described previous sections: \pkg{binsegRcpp}
timings for four loss functions, and three cases (best, worst,
flat).
In agreement with our expectations, Figure~\ref{fig:compare-distributions} shows that the Laplace median model (L1 loss) has
slightly larger asymptotic slope than the Normal/Poisson models, in the best and flat cases (but
...
We have presented \pkg{binsegRcpp}, an R package that provides an
efficient C++ implementation of binary segmentation for change-point
detection. Our theoretical and empirical comparisons with other
packages showed that \pkg{binsegRcpp} has state-of-the-art speed, and
several unique features such as cross-validation
(Table~\ref{tab:packages}). To obtain the expected time complexity in
best case data, we have emphasized the importance of storing candidate
segments to split in an appropriate container (C++ Standard Template
Library \code{multiset} or \code{priority\_queue}). Our empirical
analyses showed asymptotic differences between loss functions, and
between different implementations of binary segmentation. For future
work, we may extend \pkg{binsegRcpp} to work with multi-dimensional
data sequences, similar to \code{fpop::multiBinSeg}, \pkg{ruptures},
or \pkg{blockcpd}.

\paragraph{Reproducible Research Statement.}
Code and data for replication can be downloaded from \url{https://github.com/tdhock/binseg-model-selection}
