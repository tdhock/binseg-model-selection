...
\caption{\label{tab:overview}
  Properties (rows) of five binary segmentation libraries (columns).
...
The \code{before.mean = -3} and \code{after.mean=6} values are the means of the resulting segments, before and after that split.
The other columns, \code{invalidates.index} and \code{invalidates.after}, indicate the previous means which are no longer valid, after the given split.
...
In the third row, we see a new split at \code{end=4}, resulting in new parameters \code{before.mean=9} and \code{after.mean=3}, which invalidate the previous mean in row 2 after the split (\code{after.mean=6}).
...
However, these three values are incorrect (the correct segment means are 0, 9, and 3).
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:heap-pred} Comparing computation time of \pkg{binsegRcpp} with three containers, and two baselines: \pkg{changepoint} and \pkg{ruptures}.}
\end{figure}
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:heap-refs} Estimation of asymptotic time (seconds) and memory (kilobytes) usage for \pkg{binsegRcpp}.
    Black curves are empirical measurements, and violet curves are references.
    These data suggest that both list and multiset containers are $O(N)$;
...
\begin{figure}[t!]
  ...
  \caption{\label{fig:heap-refs} Estimation of asymptotic time (seconds) and memory (kilobytes) usage for \pkg{changepoint} and \pkg{ruptures}.
    Black curves are empirical measurements, and violet curves are references.
    These data suggest that \pkg{changepoint} is $O(N^2)$ space and $O(N^3)$ time;
    \pkg{ruptures} is $O(N)$ space and $O(N^{1.5})$ time.
  }
\end{figure}
