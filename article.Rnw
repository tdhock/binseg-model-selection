...
\Abstract{
  ...
  In practice,
  existing implementations can be asymptotically slower, and can 
  return incorrect results.
  We propose \pkg{binsegRcpp}, an \proglang{R} package which
  provides a correct \proglang{C++} implementation,
  with the expected asymptotic time complexity.
...
user-specified maximum number of change-points $K$, and we would like
to compute a sequence of models from 1 to $K$ change-points.

Using
the classical dynamic programming algorithm of
\citet{segment-neighborhood}, optimal change-points for a given cost
function, and a sequence of model sizes $k\in\{1,\dots,K\}$, can be
computed in $O(N^2 K)$ time.
%\citep{Jackson2005}
More recently, the pruned dynamic programming algorithms of
\citet{Maidstone2017} can be used to compute the same sequence of $K$
optimal models, with the same $O(N^2 K)$ worst case time complexity,
but faster $O(KN)$ time best case time complexity.  Other dynamic
...
a sub-routine to efficiently compute a range of $K$ change-point
models, using the CROCS algorithm \citep{Liehrmann2021chipseq}, which
again results in best case $O(N K)$ time, worst case $O(N^2 K)$ time.

Binary segmentation is a heuristic algorithm for computing a sequence of models from 1 to $K$ change-points \citep{binary-segmentation}.
Using a simple loss function like the square loss, the best case of binary segmentation is $O(N \log K)$ time \citep{Hocking2024finite}, which is much faster than dynamic programming.
The worst case of binary segmentation is $O(N K)$ time, which is the same speed as the best case of dynamic programming.
