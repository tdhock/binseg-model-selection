...
Figure above shows some bug in poisson loss that needs to be fixed,
probably loss for segment with only zero data.
...
Figure above showws that ruptures looks asymptotically slower in best
case. TODO need to verify these data actually achieve best case number
of candidate splits for this loss.
...
- for worst case changepoint is faster up to very large model sizes,
  but asymptotically slower. TODO need to verify that these data
  actually do achieve best/worst case for this loss.
...
- TODO need to verify that these data actually achieve best/worst case
  for this loss.
